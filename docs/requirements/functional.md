...
| FR-2.3a | **Narrative coverage analysis** — After drafting, the NarrativeCoverageAgent compares narrative pillars against the resume. Pillars backed by career evidence but not reflected in the resume are identified as coverage gaps. Concrete suggestions (drawn from career evidence) are inserted at anchor sections. Advisory only — never blocks convergence. Results are persisted as `coverage.json` and displayed in the session detail page. Falls back to keyword heuristic when the LLM is unavailable. |
