...
| FR-2.7 | **Refinement** — The `refine` operation loads the prior version's reviewer findings (all eight reviewers) and the candidacy narrative, and passes them to the RepairAgent alongside the user's free-form feedback (single pass, no loop). Reviewer findings are labeled as reference-only context — the LLM only acts on them when the user's feedback explicitly requests it (e.g. "implement all hiring-manager suggestions except X"). They are never applied autonomously. The narrative (thesis, pillars, gap framing) is included so users can reference narrative structure in their feedback (e.g. "strengthen the second pillar"). After the repair pass, all eight reviewers run once on the result. The updated document is saved as a new version with review feedback. |
| FR-2.8 | _(Reserved)_ |
