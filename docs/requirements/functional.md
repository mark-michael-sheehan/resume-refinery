...
| FR-2.5 | **Repair** — The RepairAgent fixes documents that fail verification using surgical find/replace edits (see [convergence.md](convergence.md)). Edits support three modes: find/replace (default), deletion (replace=""), and insert_after (anchor preserved, content appended). Two additional section-level operations — `remove_section` and `add_section` — are available when a section index is extracted from the document. Section ops are resolved into regular edits in Phase 0 of `apply_edits`. |
...
| FR-2.7 | **Refinement** — The `refine` operation loads the prior version's reviewer findings (all eight reviewers) and the candidacy narrative, and passes them to the RepairAgent alongside the user's free-form feedback. The repair loop runs up to `RESUME_REFINERY_MAX_REFINE_PASSES` times (default 2), retrying when edits fail to locate their target text — failed edits are surfaced as `prior_edits` context so the LLM can re-attempt with corrected find strings. The loop exits early when no edits fail. Before planning edits, the repair agent extracts a section index via a lightweight LLM call, enabling `remove_section` and `add_section` operations for structural changes. Reviewer findings are labeled as reference-only context — the LLM only acts on them when the user's feedback explicitly requests it (e.g. "implement all hiring-manager suggestions except X"). They are never applied autonomously. The narrative (thesis, pillars, gap framing) is included so users can reference narrative structure in their feedback (e.g. "strengthen the second pillar"). After the repair passes, all eight reviewers run once on the result. The updated document is saved as a new version with review feedback. |
| FR-2.8 | _(Reserved)_ |
